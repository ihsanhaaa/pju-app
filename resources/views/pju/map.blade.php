                    ...
                    kwhs.forEach(function(kwh) {
                        ...
                        var kwhMarker = L.geoJSON(kwhGeoJson, {
                            pointToLayer: function(feature, latlng) {
                                var detailButton = `<a href="/data-kwh/${kwh.id}" target="_blank" class="btn btn-sm btn-info text-white mx-1"><i class="fas fa-eye"></i> Lihat Detail</a>`;

                                var popupContent = `
                                    <table class="table table-bordered">
                                        <tr><th>Nama KWH Meter</th><td>${kwh.nama_kwh}</td></tr>
                                        <tr><th>Jenis KWH</th><td>${kwh.jenis_kwh}</td></tr>
                                        <tr>
                                ...
                                return L.marker(latlng, { icon: kwhIcon }).bindPopup(popupContent);
                            }
                        });
                        ...
                        kwh.pjus.forEach(function(lampu) {
                            ...
                            var lampuMarker = L.geoJSON(lampuGeoJson, {
                                pointToLayer: function(feature, latlng) {
                                    var detailButton = `<a href="/data-pju/${lampu.id}" target="_blank" class="btn btn-sm btn-info text-white mx-1"><i class="fas fa-eye"></i> Lihat Detail</a>`;
                                    var popupContent = `
                                        <table class="table table-bordered">
                                            <tr><th colspan="2" class="text-center">Lampu Jalan</th></tr>
                                            <tr><th>Nama Lampu</th><td>${lampu.nama_lampu}</td></tr>
                                            <tr><th>Tersambung Pada</th><td>${
                                                lampu.connected_to_kwh ? `KWH: ${kwhs.find(k => k.id === lampu.connected_to_kwh)?.nama_kwh || 'Tidak Ditemukan'}` :
                                                lampu.connected_to_pju ? `PJU: ${pjUs.find(p => p.id === lampu.connected_to_pju)?.nama_lampu || 'Tidak Ditemukan'}` :
                                                'Tidak Terhubung'
                                            }</td></tr>
                                            <tr><th>Sambungkan ke</th><td>
                                                <select id="connection-${lampu.id}" class="connection-select form-control" data-pju-id="${lampu.id}">
                                                    ...
                                                    ${pjUs.filter(p => p.id !== lampu.id).map(p => {
                                                        ...
                                                        return `<option value="pju-${p.id}" ${isSelectedPJU}>PJU: ${p.nama_lampu}</option>`;
                                                    }).join('')}
                                                </select>
                                                <button class="update-connection-btn btn btn-sm btn-success" data-pju-id="${lampu.id}">Update</button>
                                            </td></tr>
                                            <tr>
                                                <td colspan="2" class="text-center">
                                                    ${detailButton}
